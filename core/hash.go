...
func readZipMapEntryLen(buf []byte, cursor *int, readFree bool) (int, int, error) {
	...
	case 254:
		return 0, 0, errors.New("illegal zipmap item length")
	case 255:
		return -1, 0, nil
	default:
		var free byte
...
func (dec *Decoder) readZipListHash() (map[string][]byte, error) {
	...
	m := make(map[string][]byte)
	for i := 0; i < size; i += 2 {
		field, err := dec.readZipListEntry(buf, &cursor)
		if err != nil {
			return nil, err
		}
		value, err := dec.readZipListEntry(buf, &cursor)
		if err != nil {
			return nil, err
		}
		m[string(field)] = value
